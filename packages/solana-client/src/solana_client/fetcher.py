...
def get_token_prices(mints):
    ...
    for mint in mints:
        try:
            response = requests.get(f"https://api.dexscreener.com/latest/dex/tokens/{mint}", timeout=5)
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "429" in err_str or "Too Many Requests" in err_str:
                ...
                print(f"Warning: RPC does not recognize program ID in {func.__name__}. Skipping.")
                return None
            else:
                print(f"Rpc error: {e}")
                raise e
...
def fetch_holdings(client, owner_pubkey, program_id):
    ...
    try:
        ...
        response = call_with_retry(client.get_token_accounts_by_owner_json_parsed, owner_pubkey, opts)
...
def main(output_dir=None):
    ...
    sol_amount = sol_balance_resp.value / 1e9 if sol_balance_resp and sol_balance_resp.value else 0
